...
export default function VendorOrdersPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <h1 className="font-serif text-2xl font-bold text-foreground mb-6">{t.vendeur.orders.title}</h1>

      {isLoading ? (
        <p className="text-muted-foreground">{t.common.loading}</p>
      ) : orders.length === 0 ? (
        <p className="text-muted-foreground">{t.vendeur.orders.noOrders}</p>
      ) : (
            ...
            return (
              <div key={order.id} className="bg-card border border-border rounded-lg p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <p className="font-medium text-foreground">{t.vendeur.orders.orderNumber(order.id)}</p>
                ...
                <div className="flex justify-between font-semibold text-accent mb-3">
                  <span>{t.vendeur.orders.subtotal}</span>
                  <span>{mySubtotal.toLocaleString("fr-FR")} CFA</span>
                </div>

                <div className="pt-3 border-t border-border text-sm text-muted-foreground">
                  <p>{t.vendeur.orders.customerLabel(order.customer_name, order.customer_phone)}</p>
                  <p>{t.vendeur.orders.deliveryLabel(order.delivery_address)}</p>
                </div>
              </div>
            );
          })}
